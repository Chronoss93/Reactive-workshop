...
//        return emptyPublisher.switchIfEmpty(Flux.just("Hello"));
    ...
    public static Publisher<String> fallbackHelloOnError(Flux<String> failurePublisher) {
        ...
        return failurePublisher.onErrorResume(throwable -> Flux.just("Hello"));
    }
    ...
    public static Publisher<String> timeoutLongOperation(Callable<String> longRunningCall) {
       ...
       return Mono.fromCallable(longRunningCall).subscribeOn(Schedulers.elastic()).timeout(Duration.ofSeconds(1), Mono.just("Hello"));
    }
}
